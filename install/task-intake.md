...
2. **Run `mp:search` once per rephrasing.** Use each rephrasing as the query — different framings will surface different skills. (If `mp:search` is unavailable in the current harness, fall back to `sc:search`.)
