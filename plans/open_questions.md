...
### Q-005 · Expression grammar for `promote_when` / `demote_when` — RESOLVED 2026-05-29
**Decision:** **No grammar — no rules at all.** Tier movement is driven purely by usage quality, reported by the caller: a **good use promotes** the chunk one tier (`mp:learn promote`, refuse at tier 5), a **bad use demotes** it one tier (`mp:learn demote`), and a **bad use at tier 0 removes** the chunk from the pot. The `promote_when` / `demote_when` frontmatter blocks, the structured-key evaluator (`mp_eval_when`), the `mp:learn eval` scheduled sweep, and the day-based `mp:learn cleanup` are all removed. `use_count` / `last_used` stay in frontmatter as informational metadata only — they no longer gate movement.

**Rationale (user verbatim):** "days-since-used no need in this. only from good/bad usage … it should be simple if it was good use -> promote if it's bad use -> demmote / if it's already in 0 and demote -> remove". Caller maps good→`promote`, bad→`demote` directly (no new `record` verb).

**Landed in:** `mp/learn/action` (removed `mp_eval_when`/`mp_read_nested_key`/`cmd_eval`/`cmd_cleanup`; `promote` unconditional +1; `demote` is -1 or remove-at-0); `mp/lib/tier.sh:mp_new_chunk_frontmatter` (dropped rule blocks — also fixed the DSL-string bug); `mp/crud/action` scaffold (dropped rule blocks); `mp/learn/SKILL.md`; `plans/architecture.md`; `test/run-tests.sh` (PHASE5-LEARN-04..07,13..16 rewritten). 97/97 tests green.
...
**Rationale:** Defensive — when promote/demote ran in close succession on the same chunk (back-and-forth moves), the original awk-in-place edit appended a new history entry on every call even when an identical entry already existed. Intended fix: read the current `status_history:` block and check for exact-line duplication before appending. **NOTE 2026-05-29:** a review found the dedup was NOT actually implemented in `mp_append_status_history` (it appends unconditionally) and no `*_idempotent` test exists — this resolution overstates the code. Tracked as a separate defect; the append-only invariant holds but the idempotency guard does not.
